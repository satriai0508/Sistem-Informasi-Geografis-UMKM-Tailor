...
@section('main')
    <div id="map" style="height: 500px; margin: 25px;">
        <script>
            ...
            var map = L.map('map').setView([<?= $kordinats[0]->x ?>, <?= $kordinats[0]->y ?>], 12).addLayer(osm);
            ...
            <?php foreach ($kordinats as $data) { ?>
                ...
                .bindPopup('<?= $data->nama ?>'+ '<br>' + '<?= $data->alamat ?>' + '<br>' + '<?= $data->no_hp ?>' +'<br>'+
                    ...
                    '<br><a href="/peta/<?= strtolower($data->nama) ?>" class="text-decoration-none text-capitalize">Selengkapnya</a>')
                .openPopup();
            <?php } ?>
        </script>
    </div>
